...
% Upload arrow (up) starts from the level of unimpeded.north
% and aligns vertically with left-arrow.
\draw [arrow] (unimpeded.north -| left-arrow) -- node[midway, left, font=\small] {Upload} (left-arrow);

% Download arrow (down) ends at the level of unimpeded.north
% and aligns vertically with right-arrow.
\draw [arrow] (right-arrow) -- node[midway, right, font=\small] {Download} (unimpeded.north -| right-arrow);
...
\node (anesthetic) [sidebox,
    ...
    at=(unimpeded.south),
    yshift=-1.5cm,
    xshift=0.5cm,   % Adjusted for wider box
    text width=6.5cm]
{
...
% Define shifted coordinates for symmetrical arrows
\coordinate (hpc-left) at ([xshift=0.5cm]hpc.north);
\coordinate (hpc-right) at ([xshift=2cm]hpc.north);

% 1. Existing "Setup YAML" arrow to the LEFT (pointing down)
\draw [arrow] (unimpeded.south -| hpc-left) --
    node[midway, left, font=\small] {Setup YAML}
    (hpc-left);

% 2. NEW "Chains" arrow on the RIGHT (pointing UP)
\draw [arrow] (hpc-right) --
    node[midway, right, font=\small, align=center] {Nested sampling chains \\ MCMC chains}
    (unimpeded.south -| hpc-right);

\draw [arrow] (unimpeded.south -| anesthetic.north) -- node[midway, right, font=\small, text width=2cm, align=center] {Analysis Pipelines} (anesthetic.north);
